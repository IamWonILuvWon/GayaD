...
parser.add_argument('--ckpt-mode', type=str, default='auto', choices=['auto', 'none', 'init', 'resume'],
                    help='checkpoint handling: auto (init from exp/ckpt-path if available), none, init (weights only), resume (full training state)')
parser.add_argument('--ckpt-path', type=str, default=None,
                    help='checkpoint path for init/resume. If ckpt-mode=auto and set, it will init from this path.')
...
# Keep module-level shared_cfg in sync so dataloaders use CLI overrides.
cfg.shared_cfg.clear()
cfg.shared_cfg.update(shared_cfg)

def _resolve_ckpt_mode_and_path(args, dir_info):
    ckpt_mode = args.ckpt_mode
    ckpt_path = args.ckpt_path

    if ckpt_mode == 'none':
        return 'none', None
    if ckpt_mode in ('init', 'resume'):
        if not ckpt_path:
            raise ValueError("--ckpt-path is required when --ckpt-mode is 'init' or 'resume'")
        ckpt_path = os.path.expanduser(ckpt_path)
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(f"Checkpoint not found: {ckpt_path}")
        return ckpt_mode, ckpt_path

    if ckpt_path:
        ckpt_path = os.path.expanduser(ckpt_path)
        if not os.path.exists(ckpt_path):
            raise FileNotFoundError(f"Checkpoint not found: {ckpt_path}")
        return 'init', ckpt_path
    if dir_info.get('last_ckpt_path') and os.path.exists(dir_info['last_ckpt_path']):
        return 'init', dir_info['last_ckpt_path']
    return 'none', None
...
def main():
    ...
    ckpt_mode, ckpt_path = _resolve_ckpt_mode_and_path(args, dir_info)
    if ckpt_mode == 'init':
        ...
        trainer.fit(model, ckpt_path=ckpt_path, datamodule=dm)
    else:
        trainer.fit(model, datamodule=dm)
